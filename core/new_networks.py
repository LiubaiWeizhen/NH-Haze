...
from keras.layers import GlobalAveragePooling2D, GlobalMaxPooling2D, Reshape, Dense, multiply, Permute, Concatenate, Conv2D, Add, Activation, Lambda, add
...
def squeeze_excite_block(input_tensor, ratio=16):
    '''定义SENet的Squeeze-and-Excitation块'''
    # 获取输入特征图的通道数  
    channels = input_tensor._keras_shape[-1]
    # print(channels)     
    # Squeeze操作     
...
def channel_attention(input_tensor, ratio=8):     
    ...
    # 全局最大池化     
    max = GlobalMaxPooling2D()(input_tensor)     
    # 全连接层1     
    dense1 = Dense(channels//ratio, activation='relu')(avg)     
    # 全连接层2     
    dense2 = Dense(channels, activation='sigmoid')(dense1)     
    # 重塑形状     
    dense2 = Reshape((1,1,channels))(dense2)     
    # 计算加权后的特征     
    x = multiply([input_tensor, dense2])     
    return x  
    
def spatial_attention(input_tensor, kernel_size=7):     
    '''定义CBAM的空间注意力模块'''     
    # 通道数为1的卷积层     
    avg = Lambda(lambda x: K.mean(x, axis=3, keepdims=True))(input_tensor)     
    max = Lambda(lambda x: K.max(x, axis=3, keepdims=True))(input_tensor)     
    # 拼接平均值和最大值     
    concat = Concatenate(axis=3)([avg, max])     
    # 卷积     
    x = Conv2D(filters=1, kernel_size=kernel_size, strides=1, padding='same', activation='sigmoid')(concat)     
    # 计算加权后的特征     
    x = multiply([input_tensor, x])     
    return x  
...
def cbam_block(input_tensor, ratio=8, kernel_size=7):     
    '''定义CBAM块'''     
    # 通道注意力     
    x1 = channel_attention(input_tensor, ratio)     
    # 空间注意力    
    x2 = spatial_attention(input_tensor, kernel_size)     
    # 合并     
    x = add([x1, x2])     
    return x
...
def unet_spp_large_swish_generator_model():
    ...
    # inputs = squeeze_excite_block(inputs)
