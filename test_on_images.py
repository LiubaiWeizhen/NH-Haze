...
# from core.new_networks import unet_spp_large_swish_generator_model
...
weight_path = "./weights/g/generator_60_131.h5"
...
if __name__ == "__main__":
    ...
    img_src = glob.glob("data/test/*.png")
    # img_src = glob.glob("data/test20/test_A/*.png")
    ...
    for img_path in img_src:
        ...
        rgb_de_test = cv2.cvtColor(de_test, cv2.COLOR_BGR2RGBA)
        cv2.imwrite(f"{output_dir}/{img_name}.png", rgb_de_test)

        end = time.time()
        sum_t += (end - start)
        cnt+=1
        print(cnt, len(img_src), "runtime: ", end - start)
        # if cnt==10: break

    average_time = sum_t / cnt
    print("total time: ", sum_t)
